...
function checkRequirements() {
  ...
  // Ajouter cette ligne pour afficher l'URL de Mailpit après les vérifications
  displayMailpitUrl();
}
...
function checkPnpmVersion() {
  ...
  if (!currentPnpmVersion || !major || !minor) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
...
function checkNodeVersion() {
  ...
  if (!currentNodeVersion || !major || !minor) {
    console.error(
      `\x1b[31m%s\x1b[0m`,
      `You are running Makerkit from a directory that does not have Node installed. Please install Node and run "pnpm install" in your project directory.`,
    );
    process.exit(1);
  }
...
/**
 * Displays the Mailpit URL for local testing of emails
 */
function displayMailpitUrl() {
  const mailpitHost = process.env.MAILPIT_HOST || 'localhost';
  const mailpitPort = process.env.MAILPIT_PORT || '8025';
  const mailpitUrl = `http://${mailpitHost}:${mailpitPort}`;

  console.log(
    `\x1b[36m%s\x1b[0m`,
    `
    Mailpit UI available at: ${mailpitUrl}`
  );
}